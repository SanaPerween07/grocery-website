...
console.log(data);
  ...
  return (
    <div className="no-scrollbar flex-1 h-[95-vh] overflow-y-scroll flex flex-col justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img className="max-w-24 cursor-pointer" src={files[index] ? URL.createObjectURL(files[index]) : assets.upload_area} width={100} height={100} alt='' />
                            </label>
                        ))}
                    </div>
                </div>
